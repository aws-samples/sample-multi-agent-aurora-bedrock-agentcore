#!/bin/bash
# =============================================================
# Setup the AgentCore deploy project for Strands track
# Run from: ~/workshop/modules/05/strands/deploy/
# =============================================================
set -e
...
echo "=== Setting up AgentCore deploy project (Strands) ==="
...
dependencies = [
    # Pinned to the versions captured in the repo's committed uv.lock — the
    # exact tree the test harness validates. The AgentCore CodeZip build
    # resolves these server-side at deploy time; floor pins (>=) let it drift
    # to whatever is latest on PyPI (e.g. strands 1.42.0 on 2026-06-01), which
    # diverged from the tested tree and broke chart rendering. Exact pins keep
    # the deployed runtime identical to what the harness ran. See
    # feedback_version_upgrades: deploy what you tested.
