...
Create professional-looking charts with appropriate titles and labels."""
    ...
    def visualize_data(self, data: str, description: str) -> str:
        ...
        Always uses local matplotlib tools (BUILTIN_TOOLS). Charts are
        surfaced via the _captured_charts ContextVar side-channel, which the
        runtime adapter reads back — this bypasses both conversation-manager
        truncation AND the model summarising the multi-KB base64 SVG out of
        its final text.

        IMPORTANT — why this does NOT route through the gateway MCP client:
        the AgentCore runtime adapter opens the dataviz MCPClient context
        ONCE at handler init (and builds self.agent with the gateway tools).
        If visualize_data also did `with self.mcp_client:` it would re-enter
        an already-running session and the MCP SDK raises
        "the client session is currently running" on EVERY call — verified
        100% reproducible (5/5) on a clean deploy. So the chart tools run
        locally here. Gateway-enforced chart policy (Module 10's
        block_pie_charts Cedar demo) is an advanced topic handled separately;
        it is intentionally not wired through this code path.
        """
        query = f"Here's my data:\n{data}\n\n{description}"
        _captured_charts.set([])
        if not self._builtin_agent:
            try:
                from strands.tools.executors.sequential import SequentialToolExecutor
                executor = SequentialToolExecutor()
            except ImportError:
                executor = None
            self._builtin_agent = Agent(
                model=self.model,
                system_prompt=self._system_prompt,
                tools=list(BUILTIN_TOOLS),
                **({"tool_executor": executor} if executor else {})
            )
        logger.info(f"visualize_data via builtin tools: {query[:100]}")
        try:
            response = self._builtin_agent(query)
            result = str(response)
    ...
    def invoke_agent(self, query: str) -> str:
        """Invoke the agent with a query (uses gateway agent if available)."""
...
def main():
    ...
    parser.add_argument('-m', '--model', default="global.anthropic.claude-sonnet-4-6")
